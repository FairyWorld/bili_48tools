...
- list_live：查询 SNH48 直播或录播列表，支持分页；列表项的 `status` 保留上游状态值。
- get_live_detail：查询单场直播的标题、成员、时间、状态和封面等信息，不返回播放流地址；上游未提供状态字段时 `status` 为 `unknown`。
- list_performances：查询 SNH48 公演直播或录播列表，支持分页；列表项的 `status` 保留上游状态值。
